...
contract SafeGuard is ISafeGuard, AccessControlEnumerable, Ownable {
	 ...
	 */
	bytes32 public constant SAFEGUARD_ADMIN_ROLE =
		keccak256("SAFEGUARD_ADMIN_ROLE");
	...
	constructor(
		...
		require(
			_roles.length == _rolesAssignees.length,
			"SafeGuard::constructor: roles assignment arity mismatch"
		);
	...
	modifier justByRole(bytes32 _role) {
		require(
			...
			"SafeGuard: sender requires permission"
		);
		_;
	}
	...
	function setTimelock(address _timelock) public override onlyOwner {
		require(
			...
			"SafeGuard::setTimelock: Timelock address already defined"
		);
	...
	function queueTransaction(
		...
		bytes32 txHash = _queueTimelockTransaction(
			_target,
			_value,
			_signature,
			_data,
	...
	function queueTransactionWithDescription(
		...
		emit QueueTransactionWithDescription(
			txHash,
			_target,
			_value,
			_signature,
	...
	function cancelTransaction(
		...
		require(
			...
			"SafeGuard::cancelTransaction: transaction should be queued"
		);
	...
	function executeTransaction(
		...
		require(
			timelock.queuedTransactions(txHash),
			"SafeGuard::executeTransaction: transaction should be queued"
		);
	...
	function _queueTimelockTransaction(
		...
		require(
			!timelock.queuedTransactions(txHash),
			"SafeGuard::queueTransaction: transaction already queued at eta"
		);

...
describe("🚩 Testing: 🥩 Governance", async function () {
  ...
  it("Should deploy TimelockController", async function () {
    ...
    timelockContract = await deployContract(admin.signer, TimelockArtifact, [
      ...
      [executor.address, gnosisSafeWallet.address], // executorsd
    ]);
  ...
  it("Should deploy ERC20CoinUpgradeable", async function () {
  ...
  //   const gov = await ethers.getContractFactory("BlocksuranceGovernor");
  //   govContract = await gov.deploy({ gasLimit: 30000000 });
  ...
  it("Should be able to create proposal", async function () {
    ...
    const proposal = [[AddressZero], [42], [0xc0deface]];
    ...
    //   )
    // );
    // await govcoinContract.connect(proposer.signer).delegate(proposer.address);

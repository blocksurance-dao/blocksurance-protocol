...
import GnosisProxyFactoryArtifact from "@gnosis.pm/safe-contracts/build/artifacts/contracts/proxies/GnosisSafeProxyFactory.sol/GnosisSafeProxyFactory.json";
...
export const getExpectedContractAddress = async (
  ...
  const adminAddressTransactionCount =
    await deployer.signer.getTransactionCount();
...
export const generateMultisigWallet = async (
  ...
  const gnosisSafeMasterCopy = await deployContract(
    deployer.signer,
    GnosisSafeArtifact,
    []
  );
  ...
  // Create Gnosis Safe
  const gnosisSafeInterface = new ethers.utils.Interface(
  ...
  const gnosisProxyFactory = await deployContract(
    deployer.signer,
    GnosisProxyFactoryArtifact,
    []
  );

  try {
    // deploy gnosisSafe wallet
    const proxyCreationTx = await gnosisProxyFactory.createProxy(
...
async function getGnosisSafeContractFromTxEvent(
  ...
  const event = events.find(
    (eventItem: Event) =>
      eventItem.event === eventName && eventItem.address === contractAddress
  );
